    ...
    if (config.no_symbolize) {
      ...
      for (i = 0; i < input.length; i++) {
        ...
        if (input[i].indexOf("#0 ") > -1) {
          ...
          crash_loc = input[i].substring(
            input[i].indexOf("(") + 1,
            input[i].indexOf(")"),
          );
          // /home/user/asan-symbolized-linux-release-167422/chrome+0x3fe1186
          crash_loc = crash_loc.substring(
            crash_loc.lastIndexOf("/", crash_loc.lastIndexOf("/") - 1) + 1,
          );
...
function report_grinder(symbolized_asan_output, repro_file, reproname) {
  try {
    ...
    hash_quick = "";
    hash_full = "";
    for (let i = 0; i < asan_array.length; i++) {
      line = asan_array[i];
      if (
        line.indexOf(" #0 ") > -1 ||
        line.indexOf(" #1 ") > -1 ||
    ...
    grinder_log_file = `${
      config.result_dir + config.target
    }-${reproname}_grinder.txt`;
    ...
    fs.writeFile(grinder_log_file, grinder_log_data, (err) => {
      if (err) {
...
function time_format() {
  ...
  return `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}+${pad(
    d.getHours(),
  )}%3A${pad(d.getMinutes())}%3A${pad(d.getSeconds())}`;
}
...
function update_grinder(testcases_per_minute) {
  try {
    time = time_format();
    const os = require("os");
    body = `action=update_job_status&node=${os.hostname()}&tcpm=${testcases_per_minute}&key=${
      config.grinder_key
    }&time=`;
    body = encodeURI(body);
    body += time;
