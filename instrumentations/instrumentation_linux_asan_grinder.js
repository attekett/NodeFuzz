...
function report_grinder(symbolized_asan_output, repro_file, reproname) {
  try {
    ...
    console.log(
      "report_grinder hash_quick: [" + hash_quick.toString("hex") + "]",
    );
    console.log(
      "report_grinder hash_full: [" + hash_full.toString("hex") + "]",
    );
    ...
    grinder_log_file =
      config.result_dir + config.target + "-" + reproname + "_grinder.txt";
    grinder_log_data =
      "action=add_crash\nkey=" +
      config.grinder_key +
      "\nnode=" +
...
function update_grinder(testcases_per_minute) {
  try {
    ...
    var os = require("os");
    body =
      "action=update_job_status&node=" +
      os.hostname() +
      "&tcpm=" +
      testcases_per_minute +
      "&key=" +
      config.grinder_key +
      "&time=";
    body = encodeURI(body);
