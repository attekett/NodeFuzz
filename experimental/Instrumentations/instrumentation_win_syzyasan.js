...
function file_name_parse(data, current, repro,callback){
	...
	for(i=0; i<data.length; i++){
		if(data[i].indexOf('SyzyASAN error:')>-1){
	...
	file_name+=data[1].substring(data[1].length-3);
	...
	write_repro(current, repro, file_name,callback,log)
...
*/
function write_repro(current_repro, repro_file, file_name,callback,asanlog){
	...
	fs.mkdir(config.result_dir,function(error, stdout, stderr){
		...
		if(!fs.existsSync(config.result_dir+config.target+'-'+reproname+".txt")){
			...
			fs.writeFile(config.result_dir+'/folders/'+config.target+'-'+reproname+'/'+config.target+'-'+reproname+".txt", asanlog, function(err) {
				fs.writeFile(config.result_dir+config.target+'-'+reproname+".txt", asanlog, function(err) {
					if(err) {
						console.log(err);
						crashHandling=0;asanlog='';
						callback()
		...
		if(browser._closesNeeded==browser._closesGot || browserStartRetryRound > 30){
			...
			browser.stdout.on('data',function(data){
				...
				if(data.indexOf('PID=')!=-1){
					var pid=data.toString().match(/PID=\d+/)[0].split('=')[1]
					pids.push(pid)
				}
				if(crashHandling==1){
					asanlog+=data
				}
				else if(data.indexOf("SyzyASAN error:")>-1 && crashHandling==0){
			...
			browser.on('exit',function(){
				...
				if(process.platform=='win32'){
					try{clearTimeout(timeoutGetNewTestcase)}catch(e){}
	    				if(crashHandling){
							if(asanlog.indexOf("SyzyASAN error:")>-1 ){		
								console.log('Caught')
								file_name_parse(asanlog,config.previousTestCasesBuffer[0],cloneArray(config.previousTestCasesBuffer),startBrowser);
								asanlog='';
								crashHandling=0;
			...
			if(browserStartRetryRound>=30){
				...
				timeoutBrowser=setTimeout(startBrowser,2000)
		}
